...
public:
	...
	void searchOrderTravers(node* head, int num, vector<T>* res) // пр€мой обход (1)
	{
		if (head != NULL)
		{
	...
	//void preOrderTravers(node* head, CustomVector<T>* res, T low, T high) // перегрузка дл€ поиска
	//{
	//	if (head != NULL)
	//	{
	...
	void sortOrderTravers(node* head, vector<T>* res) // обход с выводом в отсортированном пор€дке (÷ентрированный)
	{
		if (head != NULL)
		{
	...
	void SearchInRange(node* head, CustomVector<T>* res, T low, T high, int& comp) // перегрузка дл€ поиска
	{
		if (head != NULL)
		{
			comp++;
